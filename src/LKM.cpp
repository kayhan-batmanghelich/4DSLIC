...
//  The k seed values are taken as uniform spatial pixel samples.
//==========================================================================
void LKM::GetKValues_VecXYZ(
                VectorImageType::Pointer                vecImg, 
                vector< vecPixelType >&                 kseedsm,
		vector<double>&				kseedsx,
		vector<double>&				kseedsy,
		vector<double>&				kseedsz,
		const int&				STEP)
{
	...
	kseedsz.resize(numseeds);
        ...
        vector< vecPixelType >::iterator it;
        cout << "alocating memory to the data vector ...";
        for ( it=kseedsm.begin() ; it < kseedsm.end(); it++ )
        { 
             it->Reserve(m_numChan) ;              
        }
        cout << " done ! " << endl;


        cout << "filling the data and coordinate vectors ...." ;
       	for( int z = 0; z < zstrips; z++ )
	{
		int ze = z*zerrperstrip;
		int d = (z*STEP+zoff+ze);
		for( int y = 0; y < ystrips; y++ )
		{
			...
			for( int x = 0; x < xstrips; x++ )
			{
				...
				//_ASSERT(n < numseeds);
                                ...
                                pixelIndex[0] = x*STEP+xoff+xe ;
                                pixelIndex[1] = y*STEP+yoff+ye ;
                                pixelIndex[2] = d ;

                                
                                // filling the data vector (kseedm)
				VectorImageType::PixelType pixelValue = vecImg->GetPixel(pixelIndex);
				kseedsm[n] = pixelValue ;
...
//===============================================================================
//         perform voxel clustering for multi-channel images
//===============================================================================
void  LKM::PerformLKMVoxelClustering(
		VectorImageType::Pointer		vecImg,
		vector<vecPixelType>&			kseedsm,
...
{
        unsigned long sz = m_width*m_height;
	const unsigned int numk = kseedsm.size();
	int numitr(0);
	...
	vector<double> inv(numk, 0);//to store 1/clustersize[k] values
        ...
        vector< double >            initChan(m_numChan,0) ;
	vector< vector <double> >   sigmam(numk, initChan) ;
	vector<double>              sigmax(numk, 0);
	vector<double>              sigmay(numk, 0);
	vector<double>              sigmaz(numk, 0);
	...
	for( int itr = 0; itr < 5; itr++ )
	{
				...
				for( int y = y1; y < y2; y++ )
				{
					for( int x = x1; x < x2; x++ )
					{
				                ...
				                VectorImageType::PixelType pixelValue = vecImg->GetPixel(pixelIndex);
                                                ...
                                                // computing distance
                                                //  VectorImageType::PixelType   diffVec ;
                                                //  diffVec = pixelValue - kseedsm[n] ;
                                                //  dist = diffVec->GetSquaredNorm() ; 
                                                dist = 0 ;
                                                for (int cnt=0;cnt<m_numChan;cnt++)
                                                {
                                                      dist = dist + (pixelValue[cnt] - kseedsm[n][cnt])*(pixelValue[cnt] - kseedsm[n][cnt]) ;
                                                }
		...
		for( int d = 0; d < m_depth; d++  )
		{
			...
			for( int r = 0; r < m_height; r++ )
			{
				for( int c = 0; c < m_width; c++ )
				{
                                        ...
                                        pixelIndex[2] = d ;
                                        VectorImageType::PixelType pixelValue = vecImg->GetPixel(pixelIndex);
					//sigmam[klabels[d][ind]] = pixelValue + sigmam[klabels[d][ind]]  ;
                                        for (int ii=0;ii<m_numChan;ii++) { (sigmam[klabels[d][ind]])[ii] = pixelValue[ii] + (sigmam[klabels[d][ind]])[ii]  ; }
					sigmax[klabels[d][ind]] += c;
					sigmay[klabels[d][ind]] += r;
					sigmaz[klabels[d][ind]] += d;
		...
		{for( int k = 0; k < numk; k++ )
		{
                        for (int ii=0;ii<m_numChan; ii++) {kseedsm[k][ii] = sigmam[k][ii]*inv[k];}
			kseedsx[k] = sigmax[k]*inv[k];
			kseedsy[k] = sigmay[k]*inv[k];
			kseedsz[k] = sigmaz[k]*inv[k];
		}}
...
//    WriteLabelVolume
//
//  Write the results as Label volume using ITK
//=============================================================================
void  LKM::WriteLabelVolume(VectorImageType::Pointer                        vecImg,
                            sidType**&       			            klabels,
                            string&                                         filename) 
{

     LabelImageType::IndexType start;
     start = vecImg->GetLargestPossibleRegion().GetIndex() ;

 
     LabelImageType::SizeType size;
     size = vecImg->GetLargestPossibleRegion().GetSize() ;
 
     LabelImageType::RegionType region(start,size);

     LabelImageType::Pointer          labelImg = LabelImageType::New() ;
     //LabelImageType::RegionType      region ;

     //region.GetSize(vecImg->GetLargestPossibleRegion().GetSize()  ) ;
     //region.GetIndex(vecImg->GetLargestPossibleRegion().GetIndex() ) 
     labelImg->SetRegions(  region ) ;
     labelImg->SetOrigin( vecImg->GetOrigin() )  ;
     labelImg->SetDirection( vecImg->GetDirection()  )  ;
     labelImg->SetSpacing( vecImg->GetSpacing()  ) ;    
     labelImg->Allocate() ;

     // fill in the label image
     typedef itk::ImageRegionIterator< LabelImageType>    IteratorType;
     IteratorType     It(labelImg, labelImg->GetLargestPossibleRegion()) ;
     It.GoToBegin() ;
     long int sz = m_height*m_width ;
     for( int d = 0; d < m_depth; d++ )
     {
	  for( int s = 0; s < sz; s++ )
	  {
     ...
     // write the label image to a file
...
//=============================================================================
///    DoSupervoxelSegmentationForMultichannelVolume
///
///  doing supervoxel segmentation for a general multi-channel image
//=============================================================================
void LKM::DoSupervoxelSegmentationForMultichannelVolume(
                                                           VectorImageType::Pointer                     vecImg,
                                                           sidType**&					klabels,
                                                           int&						numlabels,
                                                           const int					STEP,
                                                           const double cubeness )
{
	...
	vector<double>            kseedsz(0);
        ...
        //--------------------------------------------------
        m_numChan = vecImg->GetNumberOfComponentsPerPixel() ;
	m_width  = vecImg->GetLargestPossibleRegion().GetSize()[0] ;
	m_height = vecImg->GetLargestPossibleRegion().GetSize()[1] ;
	m_depth  = vecImg->GetLargestPossibleRegion().GetSize()[2] ;
	int sz = m_width*m_height;


        //--------------------------------------------------
	klabels = new sidType*[m_depth];
	for( int d = 0; d < m_depth; d++ )
	{
		klabels[d] = new sidType[sz];
		for( int s = 0; s < sz; s++ )
        ...
        //-------------------------------------------------------------------------
        GetKValues_VecXYZ(vecImg, kseedsm, kseedsx, kseedsy, kseedsz, STEP);

        //-------------------------------------------------------------------------
        PerformLKMVoxelClustering(vecImg, kseedsm, kseedsx, kseedsy, kseedsz, klabels, STEP, cubeness);
	numlabels = kseedsm.size() ;

        RelabelStraySupervoxels(m_width, m_height, m_depth, klabels, numlabels, STEP);
